...
thisDir=os.getcwd()
print(thisDir)
                                        # change driver path according to where you place geckodriver "/_internal/geckodriver.exe"
...
def instgram_block(target,successful):
    ff.get(target)
    sleep(7)
    ff.find_element(By.XPATH,'//div[@class="x1q0g3np x2lah0s"]').click()
    sleep(7)
...
for  index,row in enumerate(table_rows):
    print(f'finished {int(index/len(table_rows)*100)} %')
    ...
    try:
        ...
        print(name,instgram)
        successful=instgram_block(instgram,successful)
    except:
        pass
ff.close()
quit()
